...
# Batch trains a partially observed network with multiple samples
...
# ## Method
# """
# function train_C!(network::PONetwork, train_x::AbstractArray{Bool,3}, train_y::AbstractArray{Bool,3}, test_x::AbstractArray{Bool,3}, test_y::AbstractArray{Bool,3}; minibatch_size::Int64=0, C::Int64=5, lr::Float64=0.1, k::Float64=0.01, epochs::Int64=80)
...
#     @progress for i in 0:epochs-1

#         # Accuracy and loss
#         sigma, spikes = evaluate(network, test_x)
#         append!(accuracy_history, mean(decode(spikes[end]) .== decode(test_y)) * 100)
#         append!(loss_history, log_p(test_y, sigma[end]))
...
#             # Update parameter
#             for i in eachindex(network.layers[1:end-1])
#                 network.layers[i].bias += (k * lr / (T - 1)) .* mean([l[c] .* vec(mean(∇bias[c, i], dims=2)) for c = 1:C])
#                 network.layers[i].weights += (k * lr / S / (T - 1)) .* mean([l[c] .* ∇weights[c, i] for c = 1:C])
#             end

#             network.layers[end].bias += (lr / (T - 1)) .* mean([vec(mean(∇bias[c, end], dims=2)) for c = 1:C])
#             network.layers[end].weights += (lr / S / (T - 1)) .* mean([∇weights[c, end] for c = 1:C])
#         end
#     end
...
# Batch trains a partially observed network with multiple samples and a centered the learning signal
...
# ## Method
# """
# function train_Ĉ!(network::PONetwork, train_x::AbstractArray{Bool,3}, train_y::AbstractArray{Bool,3}, test_x::AbstractArray{Bool,3}, test_y::AbstractArray{Bool,3}; minibatch_size::Int64=0, C::Int64=5, lr::Float64=0.1, k::Float64=0.01, epochs::Int64=80)
...
# ## Method
# """
# function train_m!(network::PONetwork, input_data::AbstractArray{Bool,3}, output_data::AbstractArray{Bool,3}, label::Vector{Int64}; τ::Int64=500, ϵ::Float64=0.001, β1::Float64=0.0, β2::Float64=0.0, patience::Int64=1000)

#     samples, T, inputs = size(input_data)
#     lr = ϵ
#     k = 0.1
#     accuracy_history = []
#     loss_history = []
...
#         f_trace = [zeros(samples, size(layer.weights, 1)) for layer in network.layers]
#         g_bias = [zeros(samples, size(layer.bias)...) for layer in network.layers]
#         g_weight = [fill(zeros(size(layer.weights)...), samples) for layer in network.layers]
#         ḡ_bias = [zeros(size(layer.bias)...) for layer in network.layers]
#         ḡ_weight = [zeros(size(layer.weights)...) for layer in network.layers]
#         v_bias = copy(ḡ_bias)
#         v_weight = copy(ḡ_weight)
#         v̄_bias = copy(ḡ_bias)
#         v̄_weight = copy(ḡ_weight)


#         # Accuracy and cost

#         sigma, spikes = evaluate(network, input_data)
#         out = decode(spikes[end])
#         accuracy = mean([out[i] == label[i] for i = eachindex(out)]) * 100
#         append!(accuracy_history, accuracy)
#         loss = mean(log_p(output_data, sigma[end]))
#         append!(loss_history, loss)

#         println("Patience: $P, Max LB: $max_LB, Current LB: $loss")
...
#             trace_step!(f_trace[1], view(input_data, :, t - 1, :), network.forward_k)
#             post = spikes[1][:, t, :] - (sigma[1][:, t, :])
#             g_bias[1] += post
#             g_weight[1] += [f_trace[1][i, :] * post[i, :]' for i = 1:samples]

#             for j in eachindex(network.layers[2:end-1]) .+ 1

#                 trace_step!(f_trace[j], view(spikes[j-1], :, t - 1, :), network.forward_k)
#                 post = spikes[j][:, t, :] - (sigma[j][:, t, :])
#                 g_bias[j] += post
#                 g_weight[j] += [f_trace[j][i, :] * post[i, :]' for i = 1:samples]

#             end

#             trace_step!(f_trace[end], view(spikes[end-1], :, t - 1, :), network.forward_k)
#             post = output_data[:, t, :] - (sigma[end][:, t, :])
#             g_bias[end] += post
#             g_weight[end] += [f_trace[end][i, :] * post[i, :]' for i = 1:samples]
...
#         ĝ_bias = [mean(g_bias[j], dims=1) for j in eachindex(network.layers)]
#         ĝ_weight = [mean(g_weight[j]) for j in eachindex(network.layers)]

#         for j in eachindex(network.layers[2:end-1])

#             v_bias[j] = vec(ĝ_bias[j] .^ 2)
#             v_weight[j] = ĝ_weight[j] .^ 2


#             # Update ḡ and v̄

#             ḡ_bias[j] = β1 * ḡ_bias[j] + (1 - β1) * vec(ĝ_bias[j])
#             ḡ_weight[j] = β1 * ḡ_weight[j] + (1 - β1) * ĝ_weight[j]
#             v̄_bias[j] = β2 * v̄_bias[j] + (1 - β2) * v_bias[j]
#             v̄_weight[j] = β2 * v̄_weight[j] + (1 - β2) * v_weight[j]


#             # Update parameters

#             network.layers[j].bias += (lr * k * loss) .* ḡ_bias[j] ./ sqrt.(v̄_bias[j])
#             network.layers[j].weights += (lr * k * loss) .* ḡ_weight[j] ./ sqrt.(v̄_weight[j])

#         end

#         v_bias[end] = vec(ĝ_bias[end] .^ 2)
#         v_weight[end] = ĝ_weight[end] .^ 2


#         # Update ḡ and v̄

#         ḡ_bias[end] = β1 * ḡ_bias[end] + (1 - β1) * vec(ĝ_bias[end])
#         ḡ_weight[end] = β1 * ḡ_weight[end] + (1 - β1) * ĝ_weight[end]
#         v̄_bias[end] = β2 * v̄_bias[end] + (1 - β2) * v_bias[end]
#         v̄_weight[end] = β2 * v̄_weight[end] + (1 - β2) * v_weight[end]


#         # Update parameters

#         network.layers[end].bias += (lr * k) .* ḡ_bias[end] ./ sqrt.(v̄_bias[end])
#         network.layers[end].weights += (lr * k) .* ḡ_weight[end] ./ sqrt.(v̄_weight[end])
...
# Calculate the firing probability
# """
# function σ(layer::Layer, f_trace::AbstractMatrix{Float64}, b_trace::AbstractMatrix{Float64})
#     σ.(f_trace * layer.weights .+ b_trace .* layer.r_weight' .+ layer.bias')
# end
